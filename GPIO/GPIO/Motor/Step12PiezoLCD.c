...
//## GPIO핀에 스텝 모터의 제어 신호를 연결하여, 스텝 모터를 회전시키기
...
int main(void)
{
	...
	// **PB7에 PIEZO 연결**
	DDRD = 0xF0; // STEP0, STEP1, STEP2, STEP3을 출력 포트로 설정한다.
	...
	lcdGotoXY(0,0); //현재 커서위치를 첫번째 줄 첫번째 칸으로 이동한다.
	...
	while (1)
	{
		...
		if(buzzer_flag)
		{
			ICR1 = 7372800/DoReMi[piano]; // 버튼에 맞는 음향을 연주한다
			OCR1C = ICR1/2; // 50% 듀티비
			piano++; // piano 변수 1증가
...
SIGNAL(TIMER0_OVF_vect)
{
	...
	if(timer0Cnt == 2) // 20ms * 2 = 40ms //40ms를 얻기 위한 카운트 횟수
	{   // (1 / (7372800 / 1024)) * 144 * 2 = 40ms -> 100hz
		timer0Cnt = 0;
